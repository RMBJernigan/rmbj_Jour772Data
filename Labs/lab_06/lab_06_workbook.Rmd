...
most_pills_per_person <- county_pills_population %>%
  select(buyer_county, buyer_state, average_pills_per_year, population)%>%
  mutate(highest_pill_shipped = (average_pills_per_year/population))%>%
  arrange(desc(highest_pill_shipped))
...
#Q: which county had the highest number of pills shipped to the county per person?
#A:Norton City, VA
```
