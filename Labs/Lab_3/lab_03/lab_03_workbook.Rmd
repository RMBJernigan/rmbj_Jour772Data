...
covid_md_county_jul_aug_pct_change <- covid_county_monthly_death_rate %>%
...
# This code displays the results of the code we created above in the markdown document
  ...
  covid_md_county_jul_aug_pct_change
...
```{r}
# A: Jones
  covid_tx_county_may_aug_pct_change <- covid_county_monthly_case_rate %>%
...
covid_tx_county_may_aug_pct_change
```
...
```{r}
#Q: which Ohio county had the highest percentage change in cumulative case rate between July and August?
#A: Marion

  covid_oh_county_jul_aug_pct_change <- covid_county_monthly_case_rate %>%
  select(county, state, d_2020_07_30, d_2020_08_30) %>%
  filter(state == "Ohio") %>%
...
covid_oh_county_jul_aug_pct_change
