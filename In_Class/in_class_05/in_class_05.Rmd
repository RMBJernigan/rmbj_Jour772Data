...
```{r}
most_death_cases_state <- covid_county %>%
  group_by(state) %>%
  summarise(total_deaths = sum(deaths)) %>%
  arrange(desc(total_deaths))

# Display below
most_death_cases_state
...
```{r}
  least_death_cases_state <- covid_county %>%
  group_by(state) %>%
  summarise(total_deaths = sum(deaths)) %>%
  arrange(total_deaths)

# Display below
least_death_cases_state
...
averages_county_cases_by_state <- covid_county %>%
  group_by(state) %>%
  summarise(average_county_cases = mean(deaths)) %>%
  arrange(desc(average_county_cases))
...
#A:New Jersey and 758
```

```{r}
low_averages_county_cases_by_state <- covid_county %>%
  group_by(state) %>%
  summarise(average_county_cases = mean(deaths)) %>%
  arrange(average_county_cases)

# Display below
low_averages_county_cases_by_state
